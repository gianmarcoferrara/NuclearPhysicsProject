...
def prepare_grid(n_points, x_min, dx):
    ...
    x = np.append(x, x[n_points - 1] + dx) #another point for convenient labelling
...
e_coulomb, y_coulomb, ph_shift_coulomb = analysis(
    n_max, v_pot_1_name, e_coulomb, ph_shift_coulomb, y_coulomb,
    n_points, dx, v_pot_1, r2, r, sqrt_r
)
...
plt.plot(
    np.absolute(e_theo),
    np.absolute(e_coulomb - e_theo) / np.absolute(e_theo),
    marker='o',
    linestyle='--',
...
e_short, y_short, ph_shift_short = analysis(
    n_max, v_pot_2_name, e_short, ph_shift_short, y_short,
    n_points, dx, v_pot_2, r2, r, sqrt_r
)
...
plt.plot(
    np.absolute(e_short),
    np.absolute(e_short - e_theo) / np.absolute(e_short),
    marker='o',
    linestyle='--',
    label="coulomb",
    color='red'
...
plt.plot(
    np.absolute(e_short),
    np.absolute(e_short - e_app) / np.absolute(e_short),
    marker='o',
    linestyle='--',
    label="delta, first-order",
    color='blue'
)
...
for a_value, color in zip(a, colors):
    ...
    n_max, v_pot_3_name, e_eff_a2, ph_shift_eff_a2, y_eff_a2,
    n_points, dx, v_pot_3, r2, r, sqrt_r
    )

    e_eff_a4, y_eff_a4, ph_shift_eff_a4 = analysis(
    n_max, v_pot_4_name, e_eff_a4, ph_shift_eff_a4, y_eff_a4,
    n_points, dx, v_pot_4, r2, r, sqrt_r
    )

    #out of this scope we want to use again results for a=1, so, save them
    if a_value == 1:
    ...
    plt.plot(
    np.absolute(e_short),
    np.absolute(e_short - e_eff_a2) / np.absolute(e_short),
    marker='o',
    linestyle='--',
    label=f"a = {a_value}",
    color=color
    ...
    plt.plot(
    np.absolute(e_short),
    np.absolute(e_short - e_eff_a4) / np.absolute(e_short),
    marker='o',
    linestyle='--',
    label=f"a = {a_value}",
    color=color
    ...
    plt.plot(
    np.absolute(e_short),
    np.absolute(ph_shift_short - ph_shift_eff_a2),
    marker='o',
    linestyle='--',
    label=f"a = {a_value}",
    color=color
    ...
    plt.plot(
    np.absolute(e_short),
    np.absolute(ph_shift_short - ph_shift_eff_a4),
    marker='o',
    linestyle='--',
    label=f"a = {a_value}",
    color=color
...
plt.plot(
    ...
    marker='o',
    linestyle='--',
    label='delta - perturbation',
    color='blue'
)
plt.plot(
    np.absolute(e_short),
    np.absolute(e_short - e_eff_a4_cut1) / np.absolute(e_short),
    marker='o',
    linestyle='--',
    label='a^4 theory',
    color='red'
